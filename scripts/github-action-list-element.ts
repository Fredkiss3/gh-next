...
class ActionListElement extends HTMLElement {
  ...
  setupFocusZone() {
    ...
    this.#actionListContentItems = Array.from(
      this.querySelectorAll(".ActionList-content")
    );
  ...
  expandItem(item: HTMLElement) {
    item.dispatchEvent(
      new CustomEvent("action-menu-item-toggle", {
        bubbles: true,
        detail: { action: "expanded", item }
      })
    );
  ...
  getParentHiddenSubGroup(item: HTMLElement) {
    return item?.closest(
      "ul.ActionList--subGroup[data-hidden]"
    ) as HTMLUListElement;
  }
  ...
  addHighlightToCurrentTarget(): void {
    ...
    const listItems =
      document.querySelectorAll<HTMLLIElement>(".ActionList-item");

    for (const listItem of listItems) {
      const listItemContentLink = listItem.querySelector<HTMLAnchorElement>(
        ".ActionList-content"
      );
  ...
  removeHighlightFromActiveItems(): void {
    const activeItems = document.querySelectorAll<HTMLLIElement>(
    ...
    const activeItemGroups = document.querySelectorAll<HTMLButtonElement>(
